...
// Mock browser localStorage and document
const storage = {};
global.localStorage = {
...
// Evaluate store module code in ESM
...
${storageCode}
${storeCode}
return { GoalStore, BlockStore, SettingsStore, SessionStore, today };
`;

const fn = new Function(evalCode);
const { GoalStore, BlockStore, SettingsStore, SessionStore, today } = fn();
...
assert.strictEqual(GoalStore.isScheduledForDate(userE, '2026-08-31'), false, 'Outcome goal has no fixed daily schedule');
...
console.log('--- TEST 4: Cross-Midnight Range Calculations ---');
const t1 = BlockStore.timeToMins('08:00');
let t2 = BlockStore.timeToMins('01:00');
if (t2 <= t1) t2 += 1440;
const diff = t2 - t1;
assert.strictEqual(diff, 1020, '08:00 to 01:00 is 17 hours (1020 mins)');
assert.strictEqual(BlockStore.minsToTime(495), '08:15');
assert.strictEqual(BlockStore.minsToTime(1440), '00:00');

console.log('✓ Cross-midnight range arithmetic passed.');

console.log('--- TEST 5: Automatic Weekly Target Calculation ---');
const savedHabit = GoalStore.save({
  name: 'Custom Habit',
  type: 'habit',
  recurrenceType: 'selected_days',
  selectedDays: [0, 2, 4, 6], // 4 days
  dailyTarget: 45
});
assert.strictEqual(savedHabit.weeklyTarget, 180, '4 active days x 45 min = 180 min weekly target');
console.log('✓ Weekly target auto-calculation passed.');

console.log('================ ALL 5 TEST SUITES PASSED (100% OK) ================');
